...
date: 2020-07-13 
version: 0.1
tags: [multivariate, second, third]
...
a. Create a scatterplot of `bill_length_mm` against `bill_depth_mm` where individual species are colored and a regression line is added to each species. Add regression lines to all of your facets.
...
```{r ggyaGalton2-ex, warning = FALSE}
...
ggplot(data = penguins, aes(x = bill_depth_mm, y = bill_length_mm, colour = sex)) +
  labs(x = "Bill depth (mm)", y = "Bill length (mm)") +
  geom_point() +
  facet_grid(~species) +
  geom_smooth(method = "lm")
